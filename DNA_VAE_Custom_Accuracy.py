...
def to_int_index(some_str):
    our_output = np.array(MAXLEN)
    for i in range(len(some_str)):
        for p in chars:
            if some_str[i]== p:
                our_output[i]=chars.index(p)
    return our_output
...
def adapted_cat_accuracy(x,y):
    ...
    for i in range(n_rows):
        ...
        output_y[i]=to_int_index(integer_indiced_y[i])
    return K.mean(K.equal(K.argmax(output_x, axis=-1), K.argmax(output_y, axis=-1)))
...
vae.compile(optimizer= optimizer, loss=vae_loss, metrics=[adapted_cat_accuracy])
